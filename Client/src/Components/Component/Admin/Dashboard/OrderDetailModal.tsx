                ...
                <div className="p-6 overflow-y-auto max-h-[calc(85vh-80px)] space-y-6">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                        <div className="bg-gray-800/50 rounded-xl p-4">
                            ...
                            <p className="text-white font-bold text-lg">Table {order.tableId}</p>
                        </div>
                        <div className="bg-gray-800/50 rounded-xl p-4">
                        ...
                        <div className="bg-gray-800/50 rounded-xl p-4">
                            <p className="text-xs text-gray-500 uppercase tracking-wide mb-1">Placed At</p>
                    ...
                    {/* Items Table */}
                    <div>
                        <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wide mb-3">Order Items</h3>
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="border-b border-gray-800">
                                        ...
                                        <th className="text-left p-3 text-xs font-semibold text-gray-500">Variant</th>
                                        <th className="text-center p-3 text-xs font-semibold text-gray-500">Qty</th>
                                        <th className="text-right p-3 text-xs font-semibold text-gray-500">Price</th>
                                        <th className="text-center p-3 text-xs font-semibold text-gray-500">Status</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {order.items.map((item, idx) => (
                                        <tr key={idx} className="border-b border-gray-800/50 last:border-b-0">
                                            <td className="p-3">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <span className="text-white text-sm font-medium">{item.itemName}</span>
                                                </div>
                                            </td>
                                            <td className="p-3 text-gray-400 text-sm">
                                                {item.variant?.option ? `${item.variant.category}: ${item.variant.option}` : "—"}
                                            </td>
                                            <td className="p-3 text-center text-white text-sm font-medium">{item.quantity}</td>
                    ...
                    <div className="bg-gray-800/50 rounded-xl p-4 space-y-2">
                        ...
                        <div className="flex justify-between text-base font-bold border-t border-gray-700 pt-2">
                            ...
                            <span className="text-green-400">₹{order.totalAmount.toLocaleString("en-IN")}</span>
                        </div>
                    </div>
                </div>
            </div>
