    ...
    return (
        <div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
            ...
            <div className="absolute flex flex-col items-center">
                ...
                <span className="text-[8px] font-black uppercase text-gray-400 tracking-widest">Efficiency</span>
            </div>
        </div>
    );
};
...
const ChefAnalyticsContent = () => {
    ...
    const stats = useMemo(() => {
        if (!data?.data || !userId) return { total: 0, pending: 0, ready: 0, perfScore: 0, topItem: "-", topItemCount: 0 };
        ...
        const allItems = data.data.flatMap(order =>
            order.items
                ...
                .map(item => ({
                    ...item,
                    orderUpdatedAt: item.itemStatus === 'READY' || item.itemStatus === 'SERVING' ? (order.updatedAt || order.createdAt) : order.createdAt
                }))
        );
        ...
        return { 
            ...
            ready: readyCount, 
            perfScore: totalCount > 0 ? Math.round((readyCount / totalCount) * 100) : 0, 
    ...
    const cards = [
        { title: "Total Tasks", value: stats.total, icon: BarChart2, color: "blue", gradient: "from-blue-600 to-indigo-700", label: "Workload" },
        { title: "Ready/Served", value: stats.ready, icon: CheckCircle, color: "emerald", gradient: "from-emerald-500 to-teal-700", label: "Completed" },
        { title: "Active Prep", value: stats.pending, icon: Clock, color: "orange", gradient: "from-orange-500 to-amber-600", label: "Processing" },
        { title: "Prime Score", value: `${stats.perfScore}%`, icon: Zap, color: "purple", gradient: "from-purple-600 to-violet-800", label: "Reliability" },
    ];
        ...
        <div className="max-w-7xl mx-auto px-6 pb-12 pt-4">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
                <div>
                    ...
                    <p className="text-gray-500 font-medium mt-1 ml-1">Precision tracking for your culinary performance</p>
                </div>
            ...
            {isLoading ? (
                <div className="min-h-[500px] flex flex-col items-center justify-center gap-4 bg-white/50 rounded-[3rem] border border-gray-100 shadow-inner">
                    ...
                    <p className="text-lg font-black text-gray-400 animate-pulse tracking-widest uppercase">Analyzing Performance...</p>
                </div>
            ) : (
                <div className="space-y-8">
                    {/* Hero Stats Grid */}
                            ...
                            >
                                ...
                                <div className="relative z-10">
                                    ...
                                    <div className="space-y-1">
                                        <div className="text-xs font-black uppercase tracking-[0.2em] opacity-70 italic">{card.label}</div>
                                        <div className="text-4xl font-black">{card.value}</div>
                                        <div className="text-sm font-bold opacity-90">{card.title}</div>
                                    </div>
                                </div>
                            </motion.div>
                        ))}
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                        ...
                        >
                            <div className="mb-6 text-center">
                                <h3 className="text-xl font-black text-gray-900 tracking-tight">Completion Rate</h3>
                                <p className="text-sm font-bold text-gray-400 italic">Efficiency Score</p>
                            </div>
                            <CircularProgress value={stats.perfScore} color="text-emerald-500" size={200} />
                            <div className="mt-8 grid grid-cols-2 gap-4 w-full">
                                <div className="bg-emerald-50 rounded-3xl p-4 text-center">
                                    ...
                                    <div className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Successful</div>
                                </div>
                                <div className="bg-gray-50 rounded-3xl p-4 text-center">
                                    <div className="text-2xl font-black text-gray-400">{stats.total}</div>
                                    <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Total Aimed</div>
                                </div>
                            </div>
                        </motion.div>
                        ...
                        >
                            ...
                            <div className="relative z-10 h-full flex flex-col">
                                <div className="flex justify-between items-start mb-10">
                                    <div>
                                        <h3 className="text-2xl font-black tracking-tight mb-2">Signature Mastery</h3>
                                        <p className="text-gray-400 font-medium">Your most frequent preparation this {filter}</p>
                                    </div>
                                ...
                                <div className="flex-grow flex flex-col justify-center">
                                    {stats.topItem === "None Yet" ? (
                                        <div className="text-center py-10 opacity-50 italic">Start completing tasks to see your specialties</div>
                                    ) : (
                                        <div className="flex items-center gap-8">
                                            ...
                                            <div>
                                                <div className="text-xs font-black uppercase tracking-[0.3em] text-emerald-400 mb-1">Top Performed Item</div>
                                                <div className="text-4xl md:text-5xl font-black text-white leading-tight">{stats.topItem}</div>
                                                <div className="flex items-center gap-2 mt-4 text-emerald-400/80 font-bold">
                                                    <Target className="w-5 h-5" />
                                                    <span>{Math.round((stats.topItemCount / stats.total) * 100)}% of your total workload</span>
                                                </div>
                                            </div>
                                        </div>
                                    )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-xl shadow-gray-50">
                            ...
                            <div className="space-y-6">
                                {[
                                    { label: "Completed Tasks", value: stats.ready, total: stats.total, color: "bg-emerald-500" },
                                    { label: "In Preparation", value: stats.pending, total: stats.total, color: "bg-orange-500" },
                                ].map(row => (
                                    <div key={row.label}>
                                        <div className="flex justify-between items-end mb-3">
                                            <span className="text-sm font-black text-gray-600 uppercase tracking-tight">{row.label}</span>
                                            <span className="text-xl font-black text-gray-900">{row.value}</span>
                                        </div>
                        ...
                        {/* Efficiency Coach */}
                        <div className="bg-emerald-600 rounded-[2.5rem] p-8 text-white relative overflow-hidden group shadow-2xl shadow-emerald-200">
                            <div className="absolute -right-10 -top-10 opacity-10 group-hover:rotate-12 transition-transform duration-1000">
                                <Zap className="w-48 h-48" />
                            </div>
                            <div className="relative z-10 h-full flex flex-col">
                                <h3 className="text-xl font-black mb-4">Culinary Coach</h3>
                                <p className="text-emerald-100 font-bold leading-relaxed mb-8 italic">
                                    {stats.perfScore > 80 
                                        ? "Exceptional velocity! Your station is operating at elite standards. Keep maintaining this pace."
                                        : stats.perfScore > 50 
                                        ? "Good steady progress. Try to move those active tasks to completion to hit your 80% prime score."
                                        : "Let's pick up the pace, Chef. High efficiency starts with focus. You can do it!"}
                                </p>
                                <button className="mt-auto w-full py-4 bg-white/20 hover:bg-white text-emerald-900 font-black rounded-2xl border border-white/20 hover:border-transparent transition-all duration-300 flex items-center justify-center gap-3 group/btn">
                                    Get Full Report
