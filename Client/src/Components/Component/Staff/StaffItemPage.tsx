...
const MyItemsSection: React.FC = () => {
  ...
  // ✅ FETCH ITEMS (original)
  const { data, refetch } = useQuery<AssignedItemsResponse>({
    queryKey: ["orders", userId, currentPage, limit],
    queryFn: () =>
      getAssignedItems(restaurantId as string, userId as string),
  });

  // ✅ FETCH TOTAL ORDERS (original with today filter)
  useEffect(() => {
    const fetchTotalOrders = async () => {
      try {
        setLoading(true);
  ...
  const stats = {
    ...
    myReady:
      data?.data.filter(
        (d) =>
          d.item.assignedCookId && d.item.itemStatus === "READY",
      ).length || 0,
  };
  ...
  return (
    <div className="max-w-9xl mx-auto px-6 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <AnimatePresence>
          {Items.map(({ orderId, item, tableNumber }) => (
            <motion.button
              key={item.itemId}
              ...
              className="bg-white p-6 rounded-2xl shadow hover:shadow-lg text-left"
            >
