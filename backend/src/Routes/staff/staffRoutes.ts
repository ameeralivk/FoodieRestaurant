...
const staffAuthController = container.get<StaffController>(
...
Router.route("/getAssignedItems/:restaurantId/:chefId").get(
  orderController.getAssignedItems,
);
Router.route("/update-item").patch(
  asyncHandler(orderController.updateItemStatus),
);
Router.route("/orders/:orderId/item/:itemId/assign-cheff").patch(
  verifyAccessToken,
  asyncHandler(orderController.assignChefToItem),
);
Router.route("/assignOrder/:orderId").patch(orderController.assignOrder);

Router.route("/order/updatestatus/:orderId").patch(
...
//get the staff
Router.route("/getstaff/:staffId").get(
  asyncHandler(staffAuthController.getStaff),
);
